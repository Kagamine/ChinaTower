...
function solve (towers) {
    ...
    for (let i = 0; i < towers.length; i ++) {
        for (let j = i + 1; j < towers.length; j ++) {
            ...
            if (flag) {
                ret.push({ lon: towers[i].lon, lat: towers[i].lat, dis: getDistance(towers[i].lat, towers[i].lon, towers[j].lat, towers[j].lon), begin: { lat: towers[i].lat, lon: towers[i].lon, name: towers[i].name, type: towers[i].type, status: towers[i].status, address: towers[i].address }, end: { lat: towers[j].lat, lon: towers[j].lon, name: towers[j].name, type: towers[j].type, status: towers[j].status, address: towers[j].address } });
            }
        }
    }
    return ret;
